...
class FlagManager:
    ...
    def to_string(self) -> str:
        ...
        max_flag_length = max(len(flag.get_display_name())
                              for flag in self.__flags) + flag_names_left_padding

        max_left_padding_until_desc = 1 + max_flag_length + flag_names_left_padding

        for flag in self.__flags:
            flag_names = ' ' * flag_names_left_padding + \
...
flag_manager = FlagManager(
    "python3 ../run_test.py [required test folder name] [optional flags]")
...
flag_manager.add_flag(
    ...
    lambda file_runner_manager: [file.toggle_ir_generation(
    ) for file in file_runner_manager.get_all_files()]
)
...
flag_manager.add_flag(
    ...
    lambda file_runner_manager, file_name: file_runner_manager.get_file(
...
flag_manager.add_flag(
    ...
    lambda file_runner_manager, file_name: [file.toggle_ir_generation(
    ) for file in file_runner_manager.get_all_files_excluding(file_name)]
)
...
flag_manager.add_flag(
    ...
    lambda file_runner_manager, file_name: [file.toggle_test_running(
    ) for file in file_runner_manager.get_all_files_excluding(file_name)]
)
...
for c_file in c_files:
    ...
    if f.ir_will_generate():
        ...
        if ir_generate_exit_status != 0:
            ...
            results.append(
                f"test {c_file} failed @ IR generation - exit status {ir_generate_exit_status}")
            continue
    ...
    if f.test_will_run():
        ...
        if test_run_exit_status != 0:
            commands_did_not_fail = False
            results.append(
                f"test {c_file} failed @ running - exit status {test_run_exit_status}")
        else:
